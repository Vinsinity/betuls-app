...
(function($) {
	...
	/*
	 * Betuls Product Form Image - Dropzone
    ...
    var initializeDropzone = function() {
		$('#dropzone-form-product-image').dropzone({
			url: "image",
			...
			init: function() {
				if( $('#dropzone-form-product-image').hasClass('dz-filled') ) {
					var dropzoneObj = Dropzone.forElement("#dropzone-form-product-image"),
	...
	$(document).ready(function(){
		if( $('#dropzone-form-product-image').get(0) ) {
			initializeDropzone();
		}
	});
	...
	/*
	 * Betuls Product Attribues - Add New
	 *
	 */
	...
	/*
	 * Betuls Product Attribues - Remove
	 *
	 */
	...
	/*
	 * Betuls Product Form - Validation and Submit the form data
	 *
	 */
	var ecommerceFormValidate = function() {
		var $form = $('.betuls-product-form');
			...
			submitHandler: function(form) {
				...
				$('.ecommerce-attribute-row').each(function(){
					...
					attsArray.push({
						...
						attVisible: !!$row.find('input[name="attVisible"]').is(':checked'),
                ...
                //
				// 	if( $form.closest('.betuls-product-form-sidebar-overlay-wrapper').get(0) ) {
				// 		$('.betuls-product-form-sidebar-overlay-wrapper').removeClass('show');
				// 	} else {
				// 		setTimeout(function(){
				// 			location.reload();
				// 		}, 5000);
