@extends('layouts.admin.app', ['title' => $category->name.' Edit'])
@push('stylesheet')
...
{{--    // ecommerce-form for jquery upload--}}
    <form class="action-buttons-fixed" action="{{ route('admin.product.categories.update', ['slug' => $category->slug]) }}" method="post">
        @csrf
{{--        <div class="row mt-2">--}}
{{--            <div class="col">--}}
                            ...
                            <div class="col-lg-3-5 col-xl-4-5">
                                <div class="form-group row align-items-center mb-3">
                                    ...
                                    <div class="col-lg-7 col-xl-6">
                                        <input type="text" class="form-control form-control-modern" name="name" value="{{ $category->name }}" required />
                                    </div>
                                </div>
                                <div class="form-group row align-items-center mb-3">
                                    <label class="col-lg-5 col-xl-3 control-label text-lg-end mb-0" for="status">Status</label>
                                    <div class="col-lg-7 col-xl-6">
                                        <select class="form-control mb-3" name="status" id="status">
                                            <option value="1" {{ $category->status ? 'selected': '' }}>Active</option>
                                            <option value="0" {{ $category->status ? '': 'selected' }}>Passive</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row align-items-center mb-3">
                                    <label class="col-lg-5 col-xl-3 control-label text-lg-end mb-0" for="order">Order</label>
                                    <div class="col-lg-7 col-xl-6">
                                        <input type="number" class="form-control form-control-modern" name="order" id="order" value="{{ $category->order }}" />
                                    </div>
                                </div>
                            </div>
                        </div>
